...
class PPOTrainer(BaseRLTrainer):
    ...
    def _init_train(self, resume_state=None):
        ...
        # obs_goal = torch.cat((batch['rgb'], batch['imagegoal_sensor_v2']), dim=-1)
        # self.context_obs = deque(maxlen=16)
        # self.context_act = deque(maxlen=16)
        # self.context_obs.append(rearrange((obs_goal), "B H W C -> B 1 C H W") / 255)
    ...
    def train(self) -> None:
        ...
        #     'results/ckpt.15.pth', map_location="cpu"
        # )
    ...
    def _eval_checkpoint(
        ...
        Returns:
            None
        """
        # conf = utils.load_config('config_files/STORM.yaml')
        # logger = Logger(path=f"results")
        # action_dim = 4
        ...
        #     device=self.device
        # ).to(self.device)
        # # world_model.load_state_dict(torch.load("ckpt/world_model.pth"))
        # replay_buffer = ReplayBuffer(
        #     obs_shape=(conf.BasicSettings.ImageSize, conf.BasicSettings.ImageSize, 6),
        ...
        # obs_goal = torch.cat((batch['rgb'], batch['imagegoal_sensor_v2']), dim=-1)
        # context_obs.append(rearrange((obs_goal), "B H W C -> B 1 C H W") / 255)
    ...
    def joint_train_world_model_agent(self):

        conf = utils.load_config('config_files/STORM.yaml')
        logger = Logger(path=f"results/DreamerNavigation")
        action_dim = 4
        max_step = 100000000
        self._init_train()
        ...
        for step in tqdm.tqdm(range(max_step)):
            ...
            if step > 0:
                ...
                agent.eval()
                with torch.no_grad():
                    if len(context_act) == 0:
                        actions = torch.randint(1, 4, (conf.JointTrainAgent.NumEnvs, 1))
                    else:
                        context_latent = world_model.encode_obs(torch.cat(list(context_obs), dim=1))
                        context_acton = torch.cat(list(context_act), dim=1)

                        prior_flattened_sample, last_dist_feat = world_model.calc_last_dist_feat(
                            context_latent.to(self.device), context_acton.to(self.device)
                        )
                        actions = agent.sample_as_env_action(
            ...
            env_slice = slice(0, self.envs.num_envs)
            for index_env, act in zip(
                    range(env_slice.start, env_slice.stop), actions.cpu().unbind(0)
            ):
            ...
            replay_buffer.append(obs_goal, actions, torch.tensor(rewards_l),
                                 torch.tensor(dones, dtype=torch.float32).unsqueeze(1))
            batch = batch_obs(observations, device=self.device)
            batch = apply_obs_transforms_batch(batch, self.obs_transforms)
            obs_goal = torch.cat((batch['rgb'], batch['imagegoal_sensor_v2']), dim=-1)

            if step > 1000 and step % 128 == 0:
                # 随机抽取经验池进行世界模型的训练
                for i in range(10):
                    obs, action, reward, termination = replay_buffer.sample(
                        conf.JointTrainAgent.BatchSize,
                        conf.JointTrainAgent.DemonstrationBatchSize if conf.JointTrainAgent.UseDemonstration else 0,
                    ...
                    with torch.no_grad():
                        world_model.eval()
                        agent.eval()
                        sample_obs_goal, sample_action, sample_reward, sample_termination = replay_buffer.sample(
                            conf.JointTrainAgent.ImagineBatchSize,
                            conf.JointTrainAgent.ImagineDemonstrationBatchSize if conf.JointTrainAgent.UseDemonstration else 0,
                            conf.JointTrainAgent.ImagineContextLength)

                        imagine_latent, agent_action, imagine_reward, imagine_termination = world_model.imagine_data(
                            agent, sample_obs_goal, sample_action,
            ...
            #     reward = rewards_l[i]  # 当前环境的奖励
            #     episode_rewards[i] += reward  # 累加到该环境的累计奖励中
            #     episode_steps[i] += 1
            #     # 如果该环境的done为True，打印累计奖励并重置累计奖励
            #     if dones[i]:
            #         print(f"环境 {i} 的 episode_reward: {episode_rewards[i]}，累计步数: {episode_steps[i]}")
            #         logger.log('ActorCritic/episode_rewards', episode_rewards[i])
            #         episode_rewards[i] = 0.0  # 重置该环境的累计奖励
            #         episode_steps[i] = 0
                    # observations = self.envs.reset()
                    # batch = batch_obs(observations, device=self.device)
                    # batch = apply_obs_transforms_batch(batch, self.obs_transforms)
            ...
            if step % 10000 == 0:
                torch.save(world_model.state_dict(), f"ckpt/world_model_{step}.pth")
                # torch.save(agent.state_dict(), f"ckpt/agent_{step}.pth")
